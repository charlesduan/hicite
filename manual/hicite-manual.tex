...
general areas that may be affected:
\begin{itemize}
...
conflicts, but there are some legacy internal macro names that do not follow
this convention yet.
\end{itemize}
...
\MacroSpec\hisectlabel
\marg{label} labels the current section.

\MacroSpec\hiendsectlabel
\marg{label} labels an end of a section range.
...
\MacroSpec\fullcite \marg{ref} Informs the package to produce a
full citation of \meta{ref} by resetting memory of that reference's use.

\MacroSpec\nofullcite
\marg{ref} Cites \meta{ref} without
displaying anything, so that the next citation to that reference is a short
form.

\MacroSpec\fulltitle\marg{ref}
Resets memory of the last statutory title
cited.

\MacroSpec\addtotoa\marg{ref}
Adds \meta{ref} to the table of authorities
...
\MacroSpec\Ct Synonym for |\sentence|.

\MacroSpec\ct Synonym for |\clause|.

\MacroSpec\Ci Synonym for |\Inline|.

\MacroSpec\ci Synonym for |\inline|.

\MacroSpec\cn Synonym for |\note|.

\MacroSpec\cx Synonym for |\addtotoa|.
...
\begin{demo}
\def\NewTOACategory#1#2#3#4{#1&#2&#3&#4\\}
\begin{tabular}{llll}
\textbf{Category} & \textbf{Supercat.} & \textbf{Text} & \textbf{In TOA?} \\
\hline
\NewTOACategory{other}{other}{Other Source/s}{Y}
\NewTOACategory{const}{const}{Constitutional Provision/s}{Y}
\NewTOACategory{found}{const}{Foundational Document/s}{Y}
\NewTOACategory{statute}{edict}{Statute/s}{Y}
\NewTOACategory{regulation}{edict}{Regulation/s}{Y}
\NewTOACategory{rule}{edict}{Rule/s}{Y}
\NewTOACategory{case}{case}{Case/s}{Y}
\NewTOACategory{admin}{case}{Administrative Decision/s}{Y}
\NewTOACategory{treaty}{edict}{Treat/y/ies}{Y}
\NewTOACategory{abbrev}{abbrev}{Abbreviation/s}{N}
\end{tabular}
\end{demo}
...
\begin{grammar}
|\DefaultTOACategory|\marg{type}\marg{category}
\Example
...
own section): Use
\begin{grammar}
|\NewTOACategory|\marg{category}\marg{super}\marg{text}|{Y}|
|\DefaultTOACategory|\marg{type}\marg{category}
\Example
...
|\NewTOACategory|\marg{category}\marg{super}\marg{text}|{Y}| \\
|\DefaultTOACategory|\marg{type}\marg{category}
\Example |\NewTOACategory{mc}{edict}{Model Code/s}{Y}| \\
