...
\DescribeOption{review} Law review formatting: selects law review fonts, expects
footnotes, formats \emph{supra} cites with note numbers, and so on.


\DescribeOption{journal} Alias for \opt{review}.


\DescribeOption{journalfonts} Selects law review fonts. Same as
|\UseFontPack{review}|. Other formatting aspects are not changed.


\DescribeOption{memofonts} Selects legal memorandum fonts. Same as
|\UseFontPack{memo}|. Other formatting aspects are not changed.


\DescribeOption{memo} Legal memorandum formatting: selects memorandum fonts,
expects citations in text, and so on.


\DescribeOption{brief} Alias for \opt{memo}.


\DescribeOption{endnote} Uses endnotes rather than footnotes for formatting.


\DescribeOption{endnotes} Same as \opt{endnote}.


\DescribeOption{fullurl} URLs are displayed with their full text.


\DescribeOption{toaurl} URLs are displayed with their full text in the table of
authorities and a shortcut otherwise, as described in \sec{urls-appearance}.


\DescribeOption{linkurl} URLs are displayed as a linked word, as described in
\sec{urls-appearance}.


\DescribeOption{italcase} Case names are italicized regardless of font packs
previously chosen.


\DescribeOption{toastar} Stars are added to common references in the table of
authorities, as described in \sec{toa-star}.


\DescribeOption{somenotename} In general, if a case is named inline in text and
...
\DescribeOption{nonotename} Suppresses case names even in citations at the
start of footnotes. This is typical of most legal journals.


\DescribeOption{useopturl} Redefines the \param{opturl} parameter to be the same
as \param{url}, as described in the documentation for the former parameter.


\DescribeOption{noterefs} Cross-references to text will refer to the closest
footnote numbers. This is typical of law reviews and is the standard when the
\opt{review} option is selected.


\DescribeOption{pagerefs} Cross-references to text will refer to the page
...
As an alternative, the |hitobib| package offers a compatibility layer that
...
\hbox{}\quad \meta{param1} |=| \meta{value1}|,| \\
\hbox{}\quad \meta{param2} |=| \meta{value2}|,| \\
\hbox{}\quad \ldots \\
|}|
\end{quote}
...
macro enables this to an extent. It takes three arguments:
\begin{itemize}
...
\begin{quote}
\meta{volume} \meta{reporter} \meta{page} (\meta{date}) \\
...
\begin{quote}
\meta{journal-text} := \meta{authors} |, | \meta{title} |, | \meta{journal-id}
\\
\meta{authors} := \meta{author} [ | & | \meta{author} ] \\
...
\begin{quote}
\meta{case} := \meta{case-name} |, | \meta{case-cite} |(| \meta{court}
\meta{date} |)| \\
\meta{case-name} := \meta{p} | v. | \meta{d} \| \meta{name} \\
\meta{case-cite} := \meta{vol-rep-page} \| |No. | \meta{docket} \\
...
\begin{quote}
\meta{jrnart} := \meta{journal-text} |(| \meta{date} |)| \\
...
\begin{quote}
\meta{website} := \meta{journal-text} |(| \meta{date} |)| \\
...
\begin{quote}
\meta{book} := \meta{authors} |, | \meta{name} |(| \meta{date} |)| \\
...
\begin{quote}
\meta{courtdoc} := \meta{name} |(| \meta{date} |)| \\
...
\begin{quote}
\meta{abbrev} := \meta{name} |(| \meta{inline} |)| \\
...
\\
\meta{date-range} := \meta{date-word} |-| \meta{date-word} \\
...
|::|\meta{segment-name}|:| \meta{segment} ]* \\
...
\begin{quote}
\meta{segment} := \meta{list} [ | to | \meta{list} ] \\
\meta{list} := \meta{item} [ |, | \meta{item} ]* \\
...
When formatting a citation item, each reference can have up to four citation
forms: a full citation, a table of authorities form, a short citation, and an
\emph{id.}\ citation. Upon each citation to the reference, it is necessary to
choose among these three forms. The following rules inform that choice.
...
The case abbreviation scheme is called |name|, and the inline case abbreviation
scheme is |inlcase|.
...
Representatives'' alone are not abbreviated. The scheme is called |leg|.


\subsection{Court Documents}

Court documents use the standard abbreviation rule, with just a different
table. The scheme is called |cdoc|.
...
The abbreviation scheme is called |expl|.
...
\begin{quote}
|\MakeFontPack|\marg{name}|{| \meta{font-spec} [ |,| \meta{font-spec} ]* |}| \\
\meta{font-spec} := \meta{context} |:| \meta{font} |/| \meta{font} \\
\meta{font} := |emph| \| |empht| \| |rm| \| |sc| \\
...
The name of a case is generally defined by the following input parameters:
\begin{itemize}
\item \param{p}: The party before the ``versus,'' generally the plaintiff.
\item \param{d}: The party after the ``versus,'' generally the defendant.
\item \param{name}: A non-versus name for the case, such as ``\emph{In re}
\meta{party}.'' This will be called the ``common name'' in the discussion below.
\item \param{inline}: Optionally, the short form name.
\end{itemize}
It is possible for cases to use both the versus-party parameters and
a common name, for example in multidistrict litigation that both has lead
parties and a topical name.

These inputs are used to produce five formatted names for the case:
\begin{itemize}
\item The full title, used in full inline citations.
\item The abbreviated full title, used in full non-inline citations.
...
beginning).
\item The short form for inline citations.
\item The abbreviated short form for non-inline citations.
\end{itemize}
...
\begin{quote}
\meta{party} := \meta{text} [ |ex rel.| \meta{text} ] \\
...
\begin{quote}
\meta{name} := [ |In re| \| |Ex parte| ] \meta{text} [ |ex rel.| \meta{text} ]\\
...
\begin{center}
\def\hi@case@shortfmt@add#1{\chop@space{#1}\\}%
\begin{tabular}{l}
...
cited:
\begin{quote}
\meta{vol} \meta{rep} (\meta{nom-vol} \meta{nom-rep}) \meta{page} \\
Example: 21 U.S. (8 Wheat.)\ 1
\end{quote}
...
date given for the case. (This does not conform to other legal citation
practices but better comports with the date principles in \sec{dates-fulldate}.)
\end{itemize}
...
\begin{quote}
\meta{history} := \meta{phrase} |:| \meta{ref} \\
...
\keyparameters{
    ...
    abbreviated according to the \textsc{cdoc} scheme.}
...
\optparameters{
    ...
    ``U.S\@.'' The place will be abbreviated using the \texttt{name} scheme.}
...
which are optional other than the year:

\optparameters{
    {editor}{Names of one or more editors of the book.}
    {edtype}{The abbreviation to follow the editor names, by default ``ed.''\ or
    ``eds.''}
    {number}{A serial number for the book. The number must contain a comma,
    separating the publisher from the serial number. (Books will use serial
    numbers with no commas for another purpose.)}
...
\ttfamily
|\defletter|\{ltr\}\{\\
\hbox{}\quad author=Benjamin Franklin,\\
\hbox{}\quad to=George Washington,\\
\hbox{}\quad ...\\
\} \\
|\defbook|\{works\}\{\\
\hbox{}\quad name=Collected Works of Franklin,\\
\hbox{}\quad ...\\
\} \\
|\defcitecontainer|\{franklin-ltr\}\{\\
\hbox{}\quad citation=ltr, \\
\hbox{}\quad in=works, \\
\hbox{}\quad page=107, \\
\}
\end{quote}
...
\keyparameters{
    ...
    The journal title is given in \param{rep}, and should be unabbreviated. If
...
\keyparameters{
    ...
    The journal title is given in \param{rep}, and should be unabbreviated.}
...
title separate from a particular page title.
\bookparenparameters
...
\begin{quote}
\meta{vol} \meta{rep} \meta{page} [ \parg{year} ] \\
...
|3| becomes ``3d'') and the word ``ed.''\ will be appended to it. If the value
is a single word, then ``ed.''\ will be appended to it as well.
