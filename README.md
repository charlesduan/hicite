...
1. Package `.sty` files, also packaged for various distributions
2. A single-file user manual, `manual/hicite-manual.pdf`
...
- Source code:
  ...
  - `tex`: Supporting files such as tables which should be included with an
    installation
  - `helpers`: A directory of LaTeX package files used for compiling the
    documentation and testing the package
  - `tables`: A script for generating tables, which should not be needed unless
    the abbreviation tables change substantially

- Generated files:
  - `*.sty`: Generated package files which should be included with an
    installation
...
Alternately, you can install the `*.sty` and `tex/*` files into an appropriate
place in your filesystem.
...
Additional files required for the distribution are in the folder `tex`. To
...
flag indicating to construct the manual source code. That generated source file
is then compiled into a PDF document.
